...
These commands mirror the Docker job executed in CI: the workflow sets up Docker Buildx, builds the dedicated CI stage (linting, testing, building), and then assembles the production image while reusing the BuildKit cache. Repeating the steps locally verifies the same Buildx path that runs on GitHub Actions.
...
- [`ci.yml`](.github/workflows/ci.yml) runs on every push and pull request. A single Docker-based job enables Buildx, builds the `ci` stage in the Dockerfile (running linting, tests, and workspace builds inside the container), and then reuses those layers to assemble the production image. On pushes to `main` or tags matching `v*`, the image is pushed to GHCR while caches are saved for subsequent runs.
- [`deploy.yml`](.github/workflows/deploy.yml) triggers when the `CI` workflow completes successfully on `main` or for release tags. It re-computes the Docker metadata, exposes the published image reference to downstream jobs, and, when deployment secrets are present, connects to remote hosts over SSH to pull the updated image and restart the Compose stack.
